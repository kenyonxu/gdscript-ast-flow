# addons/gdscript_ast/gds_parser.gd
...
func _parse_fstring_expr(p_expr_text: String):
	...
	# 不走完整 parse()（期望 extends/class_name + 类体）——直接用 _parse_expression
	var saved_tokens = tokens
	var saved_pos = pos
	var saved_error = error
	var saved_count = _error_count
	tokens = sub_tokens
	pos = 0
	error = ""
	_error_count = 0
	...
	pos = saved_pos
	error = saved_error
	_error_count = saved_count
	if error != "" or expr == null:
		return null
	return expr
