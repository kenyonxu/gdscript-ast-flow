...
## Corpus Check
- 12 files · ~31,997 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 334 nodes · 332 edges · 24 communities (21 shown, 3 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9104b834`
...
3. `Phase 3: 编辑器集成 + 完整语法 设计规范` - 13 edges
4. `TreeBase` - 13 edges
5. `Phase 2: GDScript 符号解析器 实现计划` - 11 edges
6. `_parse_expr(lv)` - 11 edges
7. `GDScript 解析器 Godot 4.7 重写规范` - 10 edges
8. `GDScriptByteCodeParser (extends Node)` - 9 edges
9. `Phase 1: GDScript 基础解析管道 实现计划` - 8 edges
10. `与实际实现的差异` - 8 edges
...
Cohesion: 0.05
Nodes (38): 1.1 目标, 1.2 范围, 1.3 与 Phase 1 的关系, 2.1 入口与签名, 2.2 内部遍历架构, 2.3 文件规划, 3.1 SymbolTable — 嵌套作用域符号表, 3.2 CallGraph — 方法调用图 (+30 more)
...
Cohesion: 0.06
Nodes (30): 10.1 目标, 10.2 实现, 4.1 容器布局, 4.2 主面板 TabBar 结构, 4.3 信号中继桥（Bridge）, 4.3 模块化启动（Bootstrap）, 4.4 面板联动, 4.5 参考 LimboAI 的可视化经验 (+22 more)
...
Cohesion: 0.08
Nodes (25): Chunk 1: 数据结构 + 框架, Chunk 2: 符号表 + 作用域链, Chunk 3: 调用图 + 信号图, Chunk 4: DefUse + Lambda 捕获, Chunk 5: EditorPlugin 集成 + 验收测试, Phase 2: GDScript 符号解析器 实现计划, Task 10: 验收测试 — 10 个测试用例, Task 1: 创建 gds_analysis_result.gd — 核心数据结构 (+17 more)

### Community 14 - "Community 14"
Cohesion: 0.08
Nodes (25): Chunk A: 编辑器 UI 基础设施, Chunk B: 三个子面板 + 搜索工具, Chunk C: 语法覆盖, Chunk D: 性能 + 错误恢复, Chunk E: 验收, Phase 3: 编辑器集成 + 完整语法 实现计划, Task A1: 创建 GDSAnalysisBridge — 信号中继桥, Task A2: 创建 GDSEditorBootstrap — 模块化启动 (+17 more)
...
Cohesion: 0.25
Nodes (8): 1. 数据类架构, 2. AST 节点类型, 3. 方法调用图 — 前向引用, 4. connect 路由, 5. 表达式解析, 6. 其他 Bug 修复, 7. 文件结构差异, 与实际实现的差异
...
Cohesion: 0.40
Nodes (5): 7.1 输入接口, 7.2 输出接口, 7.3 不修改 Phase 1 代码, 7.4 错误处理约定, 七、与 Phase 1 代码的接口约定

## Knowledge Gaps
- **231 isolated node(s):** `项目概述`, `仓库`, `项目结构`, `架构：三阶段管道`, `开发命令` (+226 more)
...
- **Why does `Phase 2: GDScript 符号解析器 设计规范` connect `Community 3` to `Community 16`, `Community 23`?**
