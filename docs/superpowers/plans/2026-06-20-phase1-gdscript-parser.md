...
- [x] `gds_ast_nodes.gd` — Token.Type 枚举 (~60种) + 全部 AST 节点类 (~30种)
- [x] `gds_tokenizer.gd` — 完整词法分析器 (字符扫描+缩进+关键字+运算符)
- [x] `gds_parser.gd` — 完整语法分析器 (类/函数/语句/表达式/运算符优先级)
- [x] `plugin.gd` — EditorPlugin 工具菜单 + 分析入口
- [x] `plugin.cfg` — 插件配置 v2.0.0
- [x] `tests/test_parser.gd` — 10 个验收测试用例全部通过
- [x] 所有错误路径有 ERROR token 或 error 字符串记录，不会崩溃
- [x] 分支：直接在 `master` 上开发
...
以下差异源于 Godot 4.7 GDScript 的语法限制（与计划中基于 Godot 3.x 习惯编写的示例代码不兼容），在 `4655652` 提交中全部修复。
...
|------|------|------|
| EOS 标记 | `"\0"` (八进制转义) | `""` (空字符串) |
| 原因 | — | Godot 4 不支持 `\0`、`\x00`，仅 Unicode 转义有效；` ` 产生 NUL 字符触发 Unicode 解析错误 |

### 2. 匹配分发方式

| 项目 | 计划 | 实际 |
|------|------|------|
| 字符分发 | `match` + `..` 范围 (`"a".."z"`) | `if-elif` + 字符比较 (`c >= "a" and c <= "z"`) |
| 转义字符分发 | `match` 内 `"\\"` / `"\""` | `if-elif` |
| 原因 | — | Godot 4.7 parser 对 `match` 分支中的 `..` 范围和 `\"` 转义存在解析 bug |

### 3. 内部类引用

| 项目 | 计划 | 实际 |
|------|------|------|
| 类型注解 | `ClassNode` / `FunctionNode` 等裸名 | `GDScriptToken.ClassNode` 全路径 |
| `is` 检查 | `m is FunctionNode` | `m is GDScriptToken.FunctionNode` |
| `.new()` 调用 | `ClassNode.new()` | `GDScriptToken.ClassNode.new()` |
| 原因 | — | AST 节点是 `GDScriptToken` 的内部类；Godot 4 要求全限定名才能解析 |

### 4. 旧版文件兼容

| 文件 | 计划状态 | 实际处理 |
|------|---------|---------|
| `gds_bc_parser.gd` | [legacy] 不修改 | Godot 3→4 API 迁移 (`PoolByteArray`→`PackedByteArray`, `bytes2var`→`bytes_to_var`, `var2bytes`→`var_to_bytes`, `subarray`→`slice`) |
| `gds_ast_parser.gd` | [legacy] 不修改 | `Array.remove()`→`remove_at()`, `GDScriptToken` 内部类→`ByteToken` |

### 5. 解析器 Bug 修复

| Bug | 症状 | 修复 |
|-----|------|------|
| 首行缩进误判 | Test 1 FAIL: `extends 后需要类名` | `_check_indent`: `indent_stack==[0]` 时设基线 |
| INDENT 输出时序 | Test 3 FAIL: `非预期的令牌: PASS` | `_scan()`: 缩进检查后若 `_pending_tokens` 非空立即返回 |
| EOF DEDENT 丢失 | Test 3 FAIL | `tokenize()`: 末尾弹出所有剩余缩进级别 |
| 成员间 NEWLINE | Test 2 FAIL: `非预期的令牌: NEWLINE` | `parse()` 循环每轮前 `_skip_newlines()` |
| `void` 类型 | Test 3 FAIL: `期望 COLON` | `_parse_type()` 识别 VOID 关键字 |
| `_parse_signal` 双重消费 | Test 4 FAIL: `非预期的令牌: IDENTIFIER` | 去掉 `_match(PAREN_OPEN)` |
| `_parse_call_args` 开括号 | Test 7 FAIL: `期望 PAREN_CLOSE` | postfix handler 先 `_advance()` 消费 `(` |
| Test 10 断言类型 | Test 10 FAIL | `get_tree().process_frame` 是 `AttributeNode` |

### 6. 文件结构差异

| 计划 | 实际 |
|------|------|
| `tests/test_tokenizer.gd` | 未创建（无对应 Task） |
| — | `tests/test_parser.tscn` 新增（运行测试所需的场景文件） |
