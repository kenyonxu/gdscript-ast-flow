...
📖 **文档**：[English](README.en.md) · [用户指南](addons/gdscript_ast/docs/user_guide_cn.md) · [User Guide](addons/gdscript_ast/docs/user_guide_en.md) · [开发者指南](addons/gdscript_ast/docs/dev_guide_cn.md) · [Developer Guide](addons/gdscript_ast/docs/dev_guide_en.md)

Godot 4.7 GDScript AST 解析 + 逻辑流分析工具。以 EditorPlugin 形式集成，支持信号连接追踪、方法调用图、变量定义-使用链分析、跨文件引用、**场景/资源结构可视化**。
...
- **三视角可视化** `.tscn`/`.tres`：节点树（场景结构 + 节点详情）/ 脚本反查（哪些场景用了某脚本）/ 信号图（节点间信号连接）
- **instance 子场景展开** — 递归解析 `instance=ExtResource(...)`，合并子场景节点树（含覆盖节点 + 环检测）
- **视角联动** — 反查/信号图点节点 → 跳节点树定位
- **支持真实项目** — limboai 行为树、instance 化场景、现代 Godot 语法（`%Node` / 表达式后缀 / UID 引用 / 行续接 等）

### 编辑器集成

- 底部面板：Summary / Call Graph / Signal Flow / Def-Use / Project 五个 Tab
- 主屏 "Flow Visualizer" Tab：Scope × Graph 切换 + 度数筛选 + 图例 + 自动布局
...
1. 将 `addons/gdscript_ast/` 复制到你的 Godot 项目 `addons/` 目录
...
| 文档 | 说明 |
|------|------|
| [用户指南](addons/gdscript_ast/docs/user_guide_cn.md) | 安装、单文件分析、项目扫描、图导航、导出、场景可视化 |
| [开发者指南](addons/gdscript_ast/docs/dev_guide_cn.md) | API 参考、集成模式、作为其他插件基建 |
| [变更记录](CHANGELOG.md) | 版本变更（Added / Changed / Fixed）|
